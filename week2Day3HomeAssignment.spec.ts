...
test('Week 2 Day 3 Assignment 3', async ({ page }) => {
    ...
    await expect(page.locator(`//div[@title='${lastName}']`)).toContainText(`${lastName}`);
})
...
test('Week 2 Day 3 Edit Individual Fields', async ({ page }) => {
    ...
    await page.click('//div[@title="New"]');
    const lastName = "EditIndivi";
    await page.locator('//input[@placeholder="Last Name"]').fill(`${lastName}`);
    await page.click('//span[text()="Save"]');
    await page.click('//a[@title="Individuals"]');
    await page.click(`//a[@title='${lastName}']`);
    ...
    await page.click('//span[text()="Save"]');
    await expect(page.locator('//div[text()="Individual"]/following-sibling::div/span')).toContainText(`${firstName}`);
